...
const approvals = new Map(); // `${execId}:${nodeId}` → { resolve, info }
const APPROVAL_TIMEOUT_MS = 24 * 60 * 60 * 1000; // 24 h

function waitForApproval({ execId, nodeId, nodeName, workflowName, input }) {
  return new Promise((resolve) => {
    ...
    approvals.set(key, {
      resolve: (d) => { clearTimeout(timer); approvals.delete(key); resolve(d); },
      info: { execId, nodeId, nodeName, workflowName, input: trunc(input, 2000), since: Date.now() }
    });
  });
}

function resolveApproval(execId, nodeId, decision, comment) {
  const a = approvals.get(`${execId}:${nodeId}`);
  if (!a) return false;
  a.resolve({ decision, comment });
  return true;
}

async function notifyApproval(url, payload) {
  if (!url) return;
  const text = `✋ L'usine — la chaîne « ${payload.workflow} » attend ta validation\nAgent : ${payload.node}\nOuvre l'exécution dans L'usine pour approuver ou rejeter (délai : 24 h).`;
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 10000);
    await fetch(url, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: text, text, ...payload }), signal: ctrl.signal
  ...
  (async () => {
    ...
    try {
      ...
      for (const nodeId of order) {
        ...
        try {
          /* ---- Validation humaine : pause AVANT l'agent marqué ---- */
          if (cfg.approval) {
            log.status = 'waiting_approval';
            log.pendingInput = trunc(nodeInput, 3000);
            pushStep({ type: 'llm', text: `✋ En attente de ta validation avant d'exécuter « ${node.name} » (délai max : 24 h)` });
            broadcast({ type: 'node:waiting', execId, nodeId, name: node.name });
            saveLogs('waiting');
            notifyApproval(data.settings?.notifyWebhookUrl, {
              workflow: wf.name, node: node.name, execId, at: new Date().toISOString()
            });
            const d = await waitForApproval({ execId, nodeId, nodeName: node.name, workflowName: wf.name, input: nodeInput });
            delete log.pendingInput;
            if (d.decision === 'reject') {
              log.status = 'rejected';
              log.error = d.comment ? `Rejeté : ${d.comment}` : 'Rejeté par l\'utilisateur';
              pushStep({ type: 'llm', text: `🚫 Rejeté${d.comment ? ` — ${d.comment}` : ''}. La chaîne s'arrête.` });
              status = 'stopped';
              broadcast({ type: 'node:error', execId, nodeId, error: log.error });
              saveLogs(status);
              break;
            }
            if (d.decision === 'timeout') {
              throw new Error('Validation expirée (aucune réponse en 24 h)');
            }
            log.status = 'running';
            pushStep({ type: 'llm', text: `✅ Validé${d.comment ? ` — ${d.comment}` : ''} : l'agent démarre.` });
            broadcast({ type: 'node:start', execId, nodeId, name: node.name });
            saveLogs('running');
            if (d.comment) nodeInput += `\n\n[Consigne du validateur : ${d.comment}]`;
          }
...
module.exports = { runWorkflow, stopExecution, testAgent, callLLM, getProvider, resolveApproval, reviewExecution };
